...
FRAGMENTS = [
    ...
    {
        "key": "gem",
        "ambient": "Gem is the compiled knowledge store — the fragments database. Immutable between maintenance runs, queryable by exact key.",
        "recognition": "Gem holds all fragments, their tier content, edges, and source links. It's the crystallized knowledge base — updated during maintenance cycles (not during conversation). In the demo, gem is seeded by populate_demo.py with meta-fragments describing the system's own architecture. The artifact.sqlite file is the gem: copied to per-key session directories on first use, giving each visitor their own working copy with shared knowledge but isolated working memory.",
        "inventory": None,
    },
    {
        "key": "lens",
        "ambient": "Lens is the read/extraction tool — how the system inspects and processes source material into fragments.",
        "recognition": "Lens extracts structured knowledge from source material. In the full system, it reads conversation logs, documents, and notes, identifying key concepts and distilling them into fragment tiers. It's the 'understanding' step before crystallization into gem. The lens pipeline: source text → key extraction → tier assignment (what's ambient vs. recognition vs. inventory) → edge discovery (what connects to what). In the demo, lens isn't actively used — fragments are pre-seeded — but the architecture supports it.",
        "inventory": None,
    },
    {
        "key": "loom",
        "ambient": "Loom is the multi-agent analysis pipeline — parallel Claude calls for complex tasks that benefit from multiple perspectives.",
        "recognition": "Loom runs multiple Claude instances in parallel, each with a different analytical lens, then synthesizes their outputs. Designed for tasks like fragment compilation (where different perspectives catch different nuances), maintenance review (checking consistency from multiple angles), and deep analysis. Not used in the demo's single-turn conversation flow, but part of the planned architecture for autonomous system maintenance.",
        "inventory": None,
    },
    {
        "key": "compass",
        "ambient": "Compass is the autonomous planning system — how the maintenance agent decides what needs attention without being told.",
        "recognition": "Compass analyzes system state and generates maintenance priorities. It looks at: fragment staleness (when was knowledge last updated?), edge density (are there isolated fragments that should be connected?), working memory pressure (is the budget always full, suggesting consolidation is needed?), and decay patterns (what's being lost that maybe shouldn't be?). In the full system, compass drives the maintenance agent's work queue. Not active in the demo.",
        "inventory": None,
    },
    {
        "key": "anvil",
    ...
    {
        "key": "mirror",
        "ambient": "Mirror is the compression pipeline — how raw conversation gets distilled into lasting knowledge. The step between ephemeral and permanent.",
        "recognition": "Mirror takes conversation fragments (events, working memory items) and compresses them into updated fragment content. It's the crystallization step: raw experience → structured knowledge. Mirror runs during maintenance cycles, not during conversation. The process: identify events that reference existing fragments → extract new information → update tier content (ambient stays concise, recognition adds story, inventory captures detail) → update edges if new connections emerged. Mirror is what makes the system learn over time.",
        "inventory": None,
    },
]
...
EDGES = [
    ...
    ("gem", "fragments", "contains"),
    ("gem", "lens", "populated by"),
    ("gem", "mirror", "updated by"),
    ("lens", "gem", "writes to"),
    ("lens", "fragments", "extracts"),
    ("loom", "orchestrator", "extends"),
    ("loom", "lens", "assists"),
