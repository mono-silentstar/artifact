...
Order is inviolable:
  ...
  2. Self-State     — what I know (ambient.md)
  3. Working Memory — plans, pins, descs, patterns, thoughts, feelings
  4. Recalled       — lookup results from previous turn
  5. Recent         — conversation history (say/do/narrate), FIFO pools
  6. Current Time   — right now
  7. Hot Context    — current message, verbatim

Activation is a static file. Self-State is one file.
...
Budget (token hard caps):
  ...
  Working Memory:   1500
  Conversation:     5000  (1500 visitor / 1500 say / 1000 do / 1000 flex)
  Recall:           1000
...
DEFAULT_VISITOR_POOL = 1500        # Visitor's messages (all tags)
...
class ConversationBudget:
    ...
    Four pools, filled most-recent-first:
      visitor  — Visitor's messages (any display tag)
    ...
    say pool while do goes to the do pool. If a pool is full,
    that piece tries flex. If flex is full, it's dropped.
    """
    visitor_pool: int = DEFAULT_VISITOR_POOL
    ...
    def hard_cap(self) -> int:
        return self.visitor_pool + self.claude_say_pool + self.claude_do_pool + self.flex_reserve
...
def _extract_display_content(raw: str, actor: str) -> str | None:
    ...
    if not matches:
        ...
        # (likely from the visitor, where the whole message is display content)
...
def _load_conversation(
    ...
    visitor_remaining = budget.visitor_pool
    ...
    for row in rows:
        ...
        actor = row["actor"]
        is_claude = actor is None or actor in ("claude", "artifact")

        if not is_claude:
            # Visitor's entire message → visitor pool
            content = _extract_display_content(row["content"], actor or "visitor")
            if not content:
                continue

            tokens = _estimate_tokens(content)

            if tokens <= visitor_remaining:
                visitor_remaining -= tokens
...
def assemble(
    ...
    Conversation: FIFO pool allocation (visitor / say / do / flex).
    """
...
def render_user(package: WakePackage) -> str:
    ...
    # Hot context — visitor's message, always last
